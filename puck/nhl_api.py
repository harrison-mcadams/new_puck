...
def get_shifts_from_nhl_html(game_id, force_refresh: bool = False, debug: bool = False):
    """
    Fallback method to fetch shifts by parsing the NHL HTML 'TV' (Visitor) and 'TH' (Home) reports.
    URL Format: http://www.nhl.com/scores/htmlreports/{season}/T{V/H}{game_number}.HTM
    ...
    # Fetch proper Team IDs and metadata from Game Feed once
    ...
    # Loop through both Visitor (TV) and Home (TH) reports
    for side_code in ['TV', 'TH']:
        url = f"http://www.nhl.com/scores/htmlreports/{season}/{side_code}{game_number}.HTM"
        if debug:
        ...
        # Determine team for this specific report if it follows standard splitter
        # TV report is usually Visitor, TH is Home.
        report_team_id = away_id if side_code == 'TV' else home_id
        ...
        for i, m in enumerate(matches):
            ...
            # Determine team using Zones or Splitter or Side Code
            current_team_id = report_team_id
            if team_zones:
                best_zone = None
                for z in team_zones:
                     if z['start'] < m.start(): best_zone = z
                     else: break
                if best_zone: current_team_id = best_zone['id']
            elif home_start_idx > 0:
                current_team_id = home_id if m.start() > home_start_idx else away_id
            
            team_code = current_team_id or report_team_id
            
            if p_pos == 'UNK' and roster_map and team_code:
            ...
            for per_str, start_str, end_str in current_shifts:
                try:
                    ...
                    if start_sec is not None and end_sec is not None:
                        ...
                        clean_name = p_name.replace(',','').replace(' ','_').strip().upper().replace('&NBSP;', '')
                        # Include team ID in player ID to ensure uniqueness for players with same name/number (e.g. Sebastian Aho)
                        pid = f"{team_code}_{clean_name}_{p_num}"
                        ...
                        all_shifts.append(s_obj)
                        shifts_by_player.setdefault(pid, []).append(s_obj)
                except: pass
